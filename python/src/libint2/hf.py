from . import _libint2 as libint

import numpy as np
import scipy.linalg

def compute_1body_ints(oper, basis, params = None):
  engine = libint.Engine(oper)
...
def compute_2body_fock(D, basis):
  engine = libint.Engine(libint.Operator.coulomb)
...
class RHF:
  ...
  def __init__(self, basis, atoms):
    if not isinstance(basis, libint.BasisSet):
      basis = libint.BasisSet(basis, atoms)
    ...
    self.S = compute_1body_ints(libint.Operator.overlap, basis)
    self.T = compute_1body_ints(libint.Operator.kinetic, basis)
    self.V = compute_1body_ints(libint.Operator.nuclear, basis, atoms)
